...
const AvailabilityCalendar: React.FC<Props> = ({ lang, onClose, selectedProperty, onProceedToCheckout }) => {
    ...
    return (
        <MotionDiv 
            ...
            exit={{ opacity: 0, y: 50 }}
            className="w-full max-w-4xl mx-auto bg-white shadow-2xl rounded-[2rem] overflow-hidden flex flex-col md:flex-row h-auto md:h-[650px]"
        >
            {/* Left Side */}
                    ...
                    {isStudio && (
                        <div className="bg-white/10 p-3 rounded-sm border border-white/20 mb-6 flex gap-3">
                            ...
                            <p className="text-xs text-stone-300 leading-relaxed">
                                <strong>Studio Policy:</strong> Max 2 Adults & 1 Infant (Strictly Enforced).
                            </p>
                        </div>
                    )}
                </div>
                ...
                <div className="mt-auto space-y-6">
                     ...
                     <div className="bg-white/5 p-4 rounded-sm border border-white/10">
                        ...
                        <input 
                            ...
                            className="w-full bg-white/10 border border-white/20 rounded-sm p-2 text-white text-sm focus:outline-none focus:border-nobel-gold mb-3"
                        />
                        ...
                        <input 
                            ...
                            className="w-full bg-white/10 border border-white/20 rounded-sm p-2 text-white text-sm focus:outline-none focus:border-nobel-gold"
                        />
                    </div>
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2 bg-white/10 rounded-sm p-1 mb-2">
                            <button onClick={() => setAdults(Math.max(1, adults - 1))} className="flex-1 hover:bg-white/20 py-1">-</button>
                            <button onClick={() => setAdults(Math.min(MAX_ADULTS, adults + 1))} disabled={adults >= MAX_ADULTS} className="flex-1 hover:bg-white/20 py-1 disabled:opacity-30">+</button>
                        </div>

                        <div className="flex justify-between items-center mb-1">
                            <span className="text-sm font-bold uppercase tracking-wider">{isStudio ? 'Infant' : 'Children'}</span>
                            <span className="text-xl font-serif">{children}</span>
                        </div>
                        <div className="flex items-center gap-2 bg-white/10 rounded-sm p-1">
                            <button onClick={() => setChildren(Math.max(0, children - 1))} className="flex-1 hover:bg-white/20 py-1">-</button>
                            <button onClick={() => setChildren(Math.min(MAX_CHILDREN, children + 1))} disabled={children >= MAX_CHILDREN} className="flex-1 hover:bg-white/20 py-1 disabled:opacity-30">+</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white p-6 md:p-8 md:w-2/3 flex flex-col">
                ...
                <div className="flex-1 calendar-wrapper overflow-y-auto">
                    <style>{`
                        ...
                        .react-calendar__tile { height: 42px; border-radius: 0.5rem; font-size: 0.9rem; }
                ...
                <div className="mt-4 border-t border-stone-100 pt-4 flex flex-col md:flex-row items-center justify-between gap-4">
                    <div className="text-center md:text-left">
                        {dateRange[0] && dateRange[1] ? (
                            ...
                            <p className="text-sm text-stone-400 italic text-red-400 flex items-center gap-1">
                                <AlertCircle size={14}/> Please select check-in & check-out
                            </p>
                        )}
                    </div>
